import { BoxCollider2D, CircleCollider2D, Collider2D, Component, Contact2DType, EPhysics2DDrawFlags, EventKeyboard, EventTarget, FixedJoint2D, Input, Joint2D, KeyCode, Node, PhysicsSystem2D, PolygonCollider2D, RigidBody2D, TiledMap, Vec2, Vec3, _decorator, director, dragonBones, find, input, size, tween, v2 } from 'cc';
...
export class Player extends Component {
    ...
    start() {
        this.playAnima()
        this.armatureDisplay.addEventListener(dragonBones.EventObject.FRAME_EVENT,e => {
           if(e.name === 'jump_complete'){
            console.log('跳完成')
            this.playState = PlayState.RELAX
            this.animation.play(AnimationType.RELAX)
           }
        },this)
        // const bone = this.armature.getBone('bone_ikTarget2')
        // tween(bone.offset).to(1, new dragonBones.Transform(bone.offset.x+100,bone.offset.y+100),     
        //     {                                                              
        //         onUpdate: (target: dragonBones.Transform, ratio: number) => {                      
        //             bone.offset.x = target.x; 
        //             bone.offset.y = target.y;          
        ...
        // 地图
        const tiledMap = this.node.parent.getChildByName('TiledMap').getComponent(TiledMap)
        // 初始化地图上的所有地形
        tiledMap.getObjectGroups().forEach(item => item.getObjects().forEach(item2 => this.initFloorCollider(item2)))
        eventTarget.once('hurt', () => {
            director.emit('hurt')
        })
    }
    /**
    ...
    crawlAction = () => {
        ...
        this.animation.stop()
        this.animation.play('crawl1', 1)                                // 缓动的时长
    ...
    handUp = (e: EventKeyboard) => {
        // 监听一次动画执行结束
        eventTarget.once('anima_end', (name: AnimationType) => {
            // 如果松开按键后，是走、推、跳、休息状态，统一切换到休息状态
            if ([PlayState.WALK, PlayState.PUSH, PlayState.RELAX, PlayState.JUMP].includes(this.playState)) {
                this.playState = PlayState.RELAX
                this.playAnima()
                if (this.node.getChildByName('R1').active === false) {
                    this.node.getChildByName('R1').active = true
                }
            }else if([PlayState.CRAWL1].includes(this.playState)){
                this.isT = false;
                this.playState = PlayState.RELAX
                this.playAnima()
            }
            // 拉、拖、状态维持在动画最后一帧，保持不变
        })
    }
    /**
     * 横向移动
    ...
    playAnima = (name?: AnimationType) => {
        ...
        switch (this.playState) {
            case PlayState.RELAX:
                this.animation.fadeIn(AnimationType.RELAX, -1, 0) // 循环执行
                break;
            case PlayState.WALK:
                let temp1 = this.animation.fadeIn(AnimationType.WALK, 0, 1) // 执行一次
                temp1.timeScale = 2
                this.scheduleOnce(() => {
                    eventTarget.emit('anima_end', AnimationType.WALK)
                }, temp1.totalTime)
                break;
            case PlayState.DRAG:
                ...
                break;
            case PlayState.JUMP:
                let temp2 = this.animation.play(AnimationType.JUMP, 1)
                temp2.timeScale = 1.8
                this.scheduleOnce(() => {
                    eventTarget.emit('anima_end', AnimationType.JUMP)
                }, temp2.totalTime / temp2.timeScale)
                break;
            case PlayState.CRAWL1:
                let temp5 = this.animation.fadeIn(AnimationType.CRAWL1, 0.2,1)
                this.scheduleOnce(() => {
                    eventTarget.emit('anima_end', AnimationType.CRAWL1)
                }, temp5.totalTime / temp5.timeScale)
                break;
            case PlayState.PULL:
            ...
            case PlayState.CRAWL:
                ...
                break;
            case PlayState.PUSH:
                let temp3 = this.animation.fadeIn(AnimationType.PUSH, 0.1, 1)
                this.scheduleOnce(() => {
                    // 这里播放完成，在执行一遍动画，是因为推的过程中可能已经离开目标了，状态已经切换，就不能再维持推的动作
                    this.playAnima()
                }, temp3.totalTime / temp3.timeScale)
                break;
            case PlayState.HURT:
                let temp4 = this.animation.fadeIn(AnimationType.HURT, -1, 1)
                temp4.timeScale = 0.7
                this.scheduleOnce(() => {
                    eventTarget.emit('hurt', AnimationType.HURT)
                }, temp4.totalTime / temp4.timeScale)
                break;
            default:
                break;
        }
    ...
    */
    ...
    update(deltaTime: number) {
        // if(this.isT){
        //     this.rig2D.applyForceToCenter(v2(1000,this.rig2D.getMass()*25),true)
        // }
    }
}
